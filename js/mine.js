...
function Mine(x, y, n) {
    ...
    this.end = function() {
        ...
        mui.alert(Config.end[Config.ends < Config.end.length ? Config.ends : Config.end.length - 1]);
        Config.ends++;
        return;
    }
...
mui.plusReady(function() {
	...
	mui("a[name=start]")[0].addEventListener("tap",function(){
		// 生成一个长宽高10格的雷区
        var x = parseInt(10);
        var y = parseInt(10);
        var n = parseInt(10);
...
//	mui("#zhaohuan")[0].addEventListener("tap",function(){
//		Config.zhaohuan++;
//      console.log('召唤click')
//      if (Config.zhaohuan == 3) {
//          mui.alert('乖！真听话。\n好了，认真点，你再点三次试试吧 ヾ(≧▽≦*)o');
//      } else if (Config.zhaohuan == 6) {
//          mui.alert('快看！Σ(っ °Д °;)っ');
//          $('#shenlong').fadeIn();
//      } else if (Config.zhaohuan > 6 && Config.zhaohuan <= 10) {
//          mui.alert('点多了不会隐藏的━━∑(￣□￣*|||━━');
//      } else if (Config.zhaohuan > 10) {
//          mui.alert('好吧，被你发现了 ＝ ＝ 其实可以隐藏');
//          $('#shenlong').fadeToggle();
//      }
//	})

//  mui('input[type=checkbox]')[0].addEventListener("tap",function() {
